<?php
class Devinc_Ajaxcart_Model_Ajaxcart extends Mage_Core_Model_Abstract
{    
	...
	public function getAjaxCartQtysJs()
	{
		$html = '<script type="text/javascript">					
					...
					var dragDropProducts = ( typeof dragDropProducts != \'undefined\' && dragDropProducts instanceof Array ) ? dragDropProducts : new Array();';
		...
		if (Mage::getSingleton('customer/session')->getProductMinMax()) {
			...
			if(count($productMinMax)){
				foreach ($productMinMax as $key => $value){
					...
					if (isset($productMinMax[$key]['wishlist'])) {
						$html .= 'dragDropProducts[\''.$key.'\'] = ( typeof dragDropProducts[\''.$key.'\'] != \'undefined\' && dragDropProducts[\''.$key.'\'] instanceof Array ) ? dragDropProducts[\''.$key.'\'] : [];';
						$html .= 'dragDropProducts[\''.$key.'\'][\'wishlist\'] = \''.$productMinMax[$key]['wishlist'].'\';';
					}
					if (isset($productMinMax[$key]['compare'])) {
						$html .= 'dragDropProducts[\''.$key.'\'] = ( typeof dragDropProducts[\''.$key.'\'] != \'undefined\' && dragDropProducts[\''.$key.'\'] instanceof Array ) ? dragDropProducts[\''.$key.'\'] : [];';
						$html .= 'dragDropProducts[\''.$key.'\'][\'compare\'] = \''.$productMinMax[$key]['compare'].'\';';
					}
				}
			}
		}
